...
from . import db
from .models import User, Product, Order, Machine, Category, Machine_Product, Customer_Cart,Order_Cart_Product
...
def get_customer_products(machine_id): 
    products=Product.query.join(Machine_Product).filter(Machine_Product.machine_id==machine_id).all()    
...
def add_to_cart(customer_id, product_id): 
    ...
    cart_item=Customer_Cart.query.join(Product).join(User).filter(Product.id == product_id, User.id == customer_id).first()

    if cart_item:
        cart_item.quantity += 1
    else:
        cart_item = Customer_Cart(customer_id=customer_id, product_id = product_id, quantity=1)
...
def get_cart(user_id):
    cart = db.session.query(Customer_Cart, Product).join(Order_Cart_Product, Customer_Cart.id == Order_Cart_Product.cart_product_id, isouter=True).filter(and_(Order_Cart_Product.id.is_(None), Customer_Cart.customer_id==user_id)).distinct().all()
    
    total_price = 0
    cart_products = []
    for cart, product in cart:
        price = product.price * cart.quantity
        total_price += price
        cart_products.append({'name':product.product_name,'quantity':cart.quantity, 'price':price})
...
def submit_order(user_id):
    order_items=db.session.query(Customer_Cart).filter(Customer_Cart.customer_id==user_id).all()
    order=Order(status="pending",customer_id=user_id)
    db.session.add(order)
    db.session.commit()
    for order_item in order_items:
        order_cart_product=Order_Cart_Product(order_id=order.id,cart_product_id=order_item.id)
...
def edit_order(order_id):
    order = db.session.query(Order.id, Order.status, User.first_name).join(User, User.id==Order.customer_id).filter(Order.id==order_id).first()
    
    order_items = db.session.query(Order, Order_Cart_Product, Customer_Cart, User, Product).join(
        Order_Cart_Product, Order.id == Order_Cart_Product.order_id).join(
        Customer_Cart, Order_Cart_Product.cart_product_id == Customer_Cart.id).join(
        User, Customer_Cart.customer_id == User.id).join(
        Product, Customer_Cart.product_id == Product.id).filter(Order.id==order_id).all()
    ...
    for order, order_cart_product, customer_cart, user, product in order_items:
        ...
        total_price += price
        products.append({'name':product.product_name,'quantity':customer_cart.quantity, 'price':price})
